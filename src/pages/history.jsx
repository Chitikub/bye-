...
export default function UserActivityPage() {
  ...
  // Logic กรองข้อมูลตามช่วงเวลา (สำหรับหน้าประวัติ)
  useEffect(() => {
    if (isFavoritePage) {
  ...
  }, [timeFilter, data, isFavoritePage]);

  // 🌟 ฟังก์ชันจัดการการนำทาง (เหมือนหน้า Place Detail)
  const handleNavigate = (item) => {
    ...
    // 1. บันทึกลงประวัติการนำทางในเครื่อง
    const history = JSON.parse(localStorage.getItem("navigation_history") || "[]");
    const now = new Date().toLocaleDateString('th-TH'); // รูปแบบ DD/MM/YYYY
    
    // กรองเอาอันซ้ำออกก่อน เพื่อเอามาไว้ลำดับแรกสุด (ล่าสุด)
    ...
    // 3. ส่งสัญญาณบอก Navbar หรือส่วนอื่นให้เปลี่ยนสถานะ
      ...
      <div className={`max-w-6xl mx-auto transition-all duration-500 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
          <div className="space-y-4">
            ...
            <p className="text-sm font-bold text-[#7E7869]">พบทั้งหมด {filteredData.length} รายการ</p>
          </div>

          {!isFavoritePage && data.length > 0 && (
            <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-orange-50 gap-1">
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${timeFilter === filter.value ? 'bg-[#4A453A] text-white shadow-md' : 'text-[#7E7869] hover:bg-gray-50'}`}
                >
                  {filter.label}
                </button>
              ))}
        ...
        {/* --- Grid Content --- */}
        {loading ? (
          <div className="text-center py-20 font-bold text-[#7E7869] animate-pulse">กำลังโหลด...</div>
              ...
              {filteredData.map((item) => (
                ...
                >
                  <div className="h-56 relative overflow-hidden">
                    ...
                    <button onClick={() => handleDelete(item.id, item.name)} className="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center text-gray-400 hover:text-red-500 shadow-lg transition-all active:scale-90">
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {/* 🌟 ปุ่มนำทางที่เพิ่ม handleNavigate */}
                      <button 
          ...
          <div className="bg-white/60 backdrop-blur-xl rounded-[3rem] p-20 text-center border border-white shadow-xl">
            <Sparkles className="w-12 h-12 text-[#FF8E6E] mx-auto mb-6 opacity-50" />
            <h2 className="text-xl font-bold text-[#4A453A]">ยังไม่มีข้อมูลในส่วนนี้</h2>
            <p className="text-[#7E7869] mt-2 mb-8">ไปสำรวจสถานที่ใหม่ๆ กันเลย</p>
            <button onClick={() => navigate("/")} className="bg-[#4A453A] text-white px-10 py-4 rounded-2xl font-bold hover:bg-[#FF8E6E] transition-all">กลับหน้าแรก</button>
          </div>
        )}
      </div>
    </main>
