  ...
  return (
    ...
    <div className="p-4 sm:p-8 md:p-12 animate-fade-in font-['Kanit'] bg-[#FDF8F1] min-h-screen">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8 md:mb-12">
        <div>
          <h1 className="text-3xl md:text-5xl font-black text-[#4A453A] leading-tight">
            แดชบอร์ด
          </h1>
          <p className="text-base md:text-lg text-[#7E7869]">
            ยินดีต้อนรับกลับมา, คุณ {adminData?.firstName || "Admin"}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 mb-8 md:mb-12">
        ...
        <div
          ...
          className="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-sm border border-white hover:shadow-xl transition-all cursor-pointer"
        >
          ...
          <p className="font-bold text-[#7E7869] mb-1 text-sm md:text-base uppercase tracking-wider">
            สมาชิกทั้งหมด
          </p>
          <h3 className="text-3xl md:text-4xl font-black text-[#4A453A]">
            {stats.users.toLocaleString()}{" "}
            <span className="text-sm font-bold text-gray-300">คน</span>
          </h3>
        </div>
        ...
        <div
          onClick={() => handleTabChange("messages")}
          className="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-sm border border-white hover:shadow-xl transition-all cursor-pointer sm:col-span-2 lg:col-span-1"
        >
          ...
          <h3
            className={`text-3xl md:text-4xl font-black ${stats.contacts > 0 ? "text-[#FF8E6E]" : "text-[#4A453A]"}`}
          >
            {stats.contacts}{" "}
            <span className="text-sm font-bold text-gray-300">แชท</span>
          </h3>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] md:rounded-[3.5rem] p-4 md:p-10 shadow-sm border border-white">
        <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8 p-6 md:p-8 bg-[#FDF8F1] rounded-[1.5rem] md:rounded-[2.5rem] border border-[#EFE9D9]">
          <div className="relative">
            <div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-green-500 flex items-center justify-center text-white font-black animate-pulse shadow-lg shadow-green-500/20 text-sm md:text-base">
              LIVE
            </div>
            <div className="absolute -inset-2 bg-green-500/10 rounded-full animate-ping" />
          </div>
          <div className="text-center md:text-left">
            <p className="font-black text-[#4A453A] text-xl md:text-2xl mb-1">
